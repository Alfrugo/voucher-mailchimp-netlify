...
async function updateMailchimpMergeField({ email, code }) {
  ...
  if (!apiKey || !dc || !listId) {
    throw new Error(
      "Missing Mailchimp env vars (MAILCHIMP_API_KEY, MAILCHIMP_SERVER_PREFIX, MAILCHIMP_LIST_ID)"
    );
  }
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      merge_fields: {
        FIRSTFREE: code,
        ADDRESS: "", // <-- prevents "Please enter a complete address" from partial legacy values
      },
    }),
  });
  ...
  if (!res.ok) {
    throw new Error(
      `Mailchimp update failed (${res.status}): ${JSON.stringify(data)}`
    );
  }
...
exports.handler = async (event) => {
  try {
    ...
    // Create voucher in Voucherify (US1 endpoint) - keep as-is since it was working for you
